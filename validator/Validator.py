...
class Validator(object):
    ...
    def deleteOld(self):
        ...
        :return:
        '''
        condition = "updatetime<'%s'" % (
            (datetime.datetime.now() - datetime.timedelta(
                minutes=config.MAXTIME)).strftime('%Y-%m-%d %H:%M:%S'))
        ...
        try:
            ...
            if not r.ok:
                ...
                if not has_more:
                    ...
                    self.sqlHelper.update(SqliteHelper.tableName,
    ...
    def checkProxyType(self, proxies):
        ...
        try:
            ...
            r = requests.get(url=config.TEST_PROXY,
                             headers=config.HEADER,
                             timeout=config.TIMEOUT,
                             proxies=proxies)
            if r.ok:
                ...
                http_x_forwared_for = root.xpath(
                    './/center[2]/table/tr[8]/td[2]')[0].text
                http_via = root.xpath('.//center[2]/table/tr[9]/td[2]')[0].text
                # print
                # ip,http_x_forwared_for,http_via,type(http_via),type(http_x_forwared_for)
                if ip == self.selfip:
                    return 3
                if http_x_forwared_for is None and http_via is None:
                    return 0
                if http_via != None and http_x_forwared_for.find(
                        self.selfip) == -1:
                    return 1
...
if __name__ == '__main__':
    ...
    # v.getMyIP()
    # v.selfip
    results = {'ip': '127.0.0.1', 'port': 8118}
    v.detect_list(results)
